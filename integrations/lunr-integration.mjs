...
function indexLunrElements(canonicalUrl, addToBulk){
	return () => traverseReplace.bind(null, (node) => {
        if(node.type === "element" && node.tagName.startsWith("lunr:document")){
...
export default function createPlugin({documentFilter}){
	let config = {};
	let pathsToIndex = []
	...
	return {
		...
		hooks: {
			'astro:config:setup': (options) => {
				// options.addRenderer({
				// 	name: 'lunr-filenames',
				// 	serverEntrypoint: '@utils/lunr.js',
				// });
				// options.injectScript("page-ssr", "\nconsole.log('hello');n");
			...
			'astro:build:done': async ({pages, routes, dir}) => {
				...
				for(let {pathname} of pages) {
					let url = new URL(pathname + "/index.html", dir);
